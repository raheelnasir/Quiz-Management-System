...
function QuizComponent() {
  ...
  useEffect(() => {
    if (!loading && quizData.length > 0) {
      const marks = quizData.reduce((total, question) => total + question.marks, 0);
      setTotalMarks(marks);
    }
  }, [loading, quizData]);
  ...
  useEffect(() => {
    if (!loading && Object.keys(userAnswers).length > 0) {
      ...
      const obtained = quizData.reduce((total, question, index) => {
        const answer = userAnswers[index];
        const marks = answer === question.quizCorrectOption ? question.marks : 0;
        return total + marks;
      }, 0);
  ...
  const fetchQuizData = async () => {
    try {
      const response = await axios.get("https://localhost:7114/admin/quiz/getquiztest");
  ...
  const handleAnswerChange = (e, index, marks) => {
    setUserAnswers({ ...userAnswers, [index]: e.target.value.trim() });
  };

  const handleSubmit = async () => {
    setShowResult(true);
    if (userAuth.userName.length > 1) {
      const response = await axios.post("https://localhost:7114/admin/quiz/creatquizresults", {
        userName: userAuth.userName,
        type: "Maths",
        percentage: isNaN(correctAnswersCount / (correctAnswersCount + incorrectAnswersCount)) ? 
                    "N/A" : 
                    `${((correctAnswersCount / (correctAnswersCount + incorrectAnswersCount)) * 100).toFixed(2)}`,
      });
    }
  };
  ...
  const renderQuiz = () => {
    if (loading) {
      return (
        <div className="text-center flex-col min-h-100 flex" style={{ alignItems: "center" }}>
      ...
      <div className="mx-auto pb-5">
        <div className="max-w-lg mx-auto mt-10">
          ...
          <div className="bg-gray-100 p-6 rounded-lg mb-6">
            ...
            <p>{isNaN(correctAnswersCount / (correctAnswersCount + incorrectAnswersCount)) ? 
                    "N/A" : 
                    `${((correctAnswersCount / (correctAnswersCount + incorrectAnswersCount)) * 100).toFixed(2)}%`}
            </p>
          </div>
            ...
            >
              ...
              {showResult && (
                <>
                  ...
                  <p className=" font-extrabold">Topic : {question.topic}</p>
                  <p>
                    Learning Resources:{" "}
                    <a
